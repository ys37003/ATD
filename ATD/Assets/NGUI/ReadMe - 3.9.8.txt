...
- NEW: Added a new tool 00 Prefab Toolbar. It lets you drop prefabs onto it for easy preview.
...
- NEW: Recreated the Chat Window example 00 it now features a resizable chat window.
...
- NEW: UICamera.currentScheme tells you the current control scheme 00 mouse, touch, or controller.
...
- NEW: Added UIButtonActivate 00 an extremely simple script that can be used to activate or deactivate something on click.
...
2.0.4:
- NEW: Added UIButton 00 same as UIButtonColor, but has a disabled state.
...
- NEW: Reorganized all examples to be in a more logical order 00 starting with the basic, common functionality and going up from there.
...
- NEW: Added a new feature to UIPanel 00 the ability to write to depth before any geometry is drawn. This doubles the draw calls but saves fillrate.
...
1.62:
- NEW: Added a new class 00 UITable 00 that can be used to organize its children into rows/columns of variable size (think HTML table).
...
1.48:
- NEW: Added a new container class 00 BetterList<>. It replaced the generic List<> in many cases, eliminating GC spikes.
